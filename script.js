...
function showIssues(issues) {
    ...
    if (issues.length === 0) {
        grid.innerHTML = "<p class='text-center py-10 text-gray-400'>No issues found</p>";
        return;
    }
...
function createCard(issue) {
    ...
    // open hole green border, closed hole purple border
    ...
    // open hole green dot, closed hole purple dot
    let dotColor = "#22c55e";
    ...
    labels.forEach(function (l) {
        let lBg = "#f3f4f6";
        let lText = "#374151";
        if (l.toUpperCase() === "BUG") {
            lBg = "#fee2e2";
            lText = "#dc2626";
        } else if (l.toUpperCase() === "HELP WANTED") {
            lBg = "#f3e8ff";
            lText = "#9333ea";
        } else if (l.toUpperCase() === "ENHANCEMENT") {
            lBg = "#dcfce7";
            lText = "#16a34a";
        } else if (l.toUpperCase() === "GOOD FIRST ISSUE") {
            lBg = "#dbeafe";
            lText = "#2563eb";
        } else if (l.toUpperCase() === "DOCUMENTATION") {
            lBg = "#fef3c7";
            lText = "#d97706";
        }
        labelsHTML += `<span style="background:${lBg}; color:${lText}; font-size:10px; font-weight:700; padding:2px 8px; border-radius:20px;">${l}</span>`;
    });
    ...
    card.innerHTML = `
        ...
        <h3 class="font-bold text-sm text-gray-900 leading-snug">${title}</h3>

        <p class="text-xs text-gray-500 leading-relaxed line-clamp-2">${description}</p>
...
// search input e type korle kono kaj hobe na
// shudhu New Issue button a click korle search hobe
...
async function doSearch(query) {
    ...
    hideSpinner();
    showIssues(results);
    document.getElementById("issues-count-text").textContent = results.length + " Issues";
}
...
function fillModal(issue) {
    ...
    if (status.toLowerCase() === "open") {
        ...
        statusEl.className = "text-xs font-bold px-3 py-1 rounded-full bg-green-100 text-green-700 border border-green-300";
    } else {
        statusEl.textContent = "● Closed";
        statusEl.className = "text-xs font-bold px-3 py-1 rounded-full bg-purple-100 text-purple-700 border border-purple-300";
    }
    ...
    labels.forEach(function (l) {
        ...
        labelsHTML += `<span class="text-xs font-bold px-2 py-0.5 rounded-full ${labelColor}">${l}</span>`;
    });
    ...
    // assignee - API te different field name thakte pare
    const assignee = issue.assignee || issue.assignedTo || issue.assigned_to || "—";
    document.getElementById("modal-assignee").textContent = assignee;

    // priority badge show koro
...
// new issue button a click korle search hobe
